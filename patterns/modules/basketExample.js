var basketModule = (function () {
	// privates
	var basket = [];

	function doSomethingPrivate() {
		// ...
	};

	function doSomethingElsePrivate() {
		// ...
	};

	// Return an object exposed to the public
	return {
		// Add items to our basket
		addItem: function( values ) {
			basket.push( values );
		},

		// Get the count of items in the basket
		returnItemCount: function () {
			return basket.length;
		},

		// Public alias to a private funciton
		doSomething: doSomethingPrivate,

		// Get the total value of the items in the basket
		getTotal: function() {
			var q = this.returnItemCount();
			var p = 0;

			while (q--) {
				p+=basket[q].price;
			}

			return p;
		}

	};

}) ();

// basket module returns an object with a public API we can use
basketModule.addItem({
	item: "bread",
	price: 0.5
});

basketModule.addItem({
	item: "butter",
	price: 0.3
});

// outputs: 2
console.log( basketModule.returnItemCount() );

// outputs: 0.8
console.log ( basketModule.getTotal() );

// However the following will not work:

// Outputs: undefiend
// This is because the basket itself is not exposed as part of our
// public API
// console.log( basketModule.basket );

// This also won't work because the basket only exists within the scope of our
// basketModule closure, but not in the returned public object
// console.log( basket ); 
